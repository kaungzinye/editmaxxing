...
type Props = { versions: VideoVersion[]; body: Clip; progress: EditProgress; keptIds: string[] | null; activeId: string | null; onKeptChange: (ids: string[]) => void; onActiveChange: (id: string) => void; onBack: () => void; onEditHooks: () => void; onNewVideo: () => void; onRender: (kind: 'draft' | 'export') => void; onSave: (id: string) => Promise<void>; onRefresh: () => void; onEditBody: () => void };
...
function Playable({ uri, text, autoplay, onEnd }: { uri: string; text?: string; autoplay: boolean; onEnd: () => void }) {
  ...
  return <>
    ...
    {error && <View style={[StyleSheet.absoluteFill, s.playError]}><Text style={s.lightText}>Refresh video links to retry playback.</Text></View>}
  </>;
}

function Preview({ version, height, onRefresh }: { version: VideoVersion; body: Clip; height: number; onRefresh: () => void }) {
  return <View style={s.previewArea}><View style={[s.preview, { height, width: height * 9 / 16 }]}>
    {version.renderUri ? <Playable key={version.renderUri} uri={version.renderUri} autoplay={false} onEnd={() => {}} />
      : <View style={s.playError}><Text style={s.lightText}>Render previews to watch the edited video.</Text></View>}
  </View><Pressable onPress={onRefresh}><Text>Refresh video links</Text></Pressable></View>;
}

export default function FinalVideoScreen({ versions, body, progress, keptIds, activeId, onKeptChange, onActiveChange, onBack, onEditHooks, onNewVideo, onRender, onSave, onRefresh, onEditBody }: Props) {
  ...
  const pairing = (v: VideoVersion) => `V${verbalIds.indexOf(v.spoken.id) + 1} + T${textIds.indexOf(v.onScreen.id) + 1}`;
  useEffect(() => {
    if (!sheet) return;
  ...
  async function save(v: VideoVersion) {
    setDownloadError('');
    try { await onSave(v.id); } catch (error) { setDownloadError(error instanceof Error ? error.message : String(error)); }
  }
  ...
  return <View style={s.root} onLayout={event => setAvailableHeight(event.nativeEvent.layout.height)}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.content}>
      ...
      <Preview key={version.id} version={version} body={body} onRefresh={onRefresh} height={Math.min(340, Math.max(200, availableHeight - (progress.percent < 100 ? 475 : 433)))} />
      <View style={s.row}><Pressable onPress={() => onRender('draft')} style={s.edit}><Text style={s.editText}>Render previews</Text></Pressable>
        <Pressable onPress={onEditBody} style={s.edit}><Text style={s.editText}>Edit shared body</Text></Pressable></View>
    ...
    <View style={s.footer}><View style={s.row}><Text style={s.selectedText}>{selected.length} of {versions.length} selected</Text><Text style={s.previewNote}>{selected.every(v => !!v.exportUri) && selected.length ? 'Ready to save' : 'Export required'}</Text></View><Pressable accessibilityRole="button" aria-disabled={!selected.length} accessibilityState={{ disabled: !selected.length }} disabled={!selected.length} style={[s.cta, !selected.length && s.disabled]} onPress={() => { setDownloadError(''); setSheet(true); onRender('export'); }}><Text style={s.downloadIcon}>↓</Text><Text style={s.ctaText}>Download selected{selected.length ? ` (${selected.length})` : ''}</Text></Pressable></View>
    </View>
