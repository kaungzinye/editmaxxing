...
type CaptureProps = {
  ...
  initialClip?: Clip; confirmLabel: string; onUse: (clip: Clip) => Promise<void>;
...
function Capture({ title, subtitle, line, movement, initialClip, confirmLabel, onUse, onBack, progress }: CaptureProps) {
  ...
  const [frameSpace, setFrameSpace] = useState({ width: 0, height: 0 });
  const frameWidth = Math.max(0, Math.min(frameSpace.width - 16, frameSpace.height * 9 / 16));
  const [actionStart, setActionStart] = useState(initialClip?.actionStartMs === undefined ? '' : String(initialClip.actionStartMs / 1000));
  ...
  async function enableCamera() {
    if (Platform.OS === 'web') { setError('To record, open the mobile app. Open the iOS development build to process a recording.'); return; }
    try {
    ...
    } catch { setError('Camera unavailable. Select a video from your library.'); }
  }
  ...
  const busy = recording || countdown !== null || saving;
  async function confirm() {
    if (!clip || saving) return;
    setSaving(true); setError('');
    try {
      const actionStartMs = movement ? Math.round(Number(actionStart) * 1000) : undefined;
      if (movement && (!actionStart.trim() || !Number.isFinite(actionStartMs) || actionStartMs! < 0 || actionStartMs! >= clip.duration * 1000)) {
        throw new Error('Enter the action start in seconds after reviewing the take. Use 0 if it starts with the recording.');
      }
      await onUse({ ...clip, actionStartMs });
  ...
  return <View style={s.screen}>
    <View style={s.header}>
      ...
      <View style={s.badge}><View style={[s.statusDot, recording && { backgroundColor: RED }]} /><Text style={s.badgeText}>{recording ? 'REC' : '9:16'}</Text></View>
    </View>
    {progress}
    <View style={s.frameSpace} onLayout={({ nativeEvent: { layout } }) => setFrameSpace({ width: layout.width, height: layout.height })}>
    <View testID="capture-frame" style={[s.viewfinder, { width: frameWidth, aspectRatio: 9 / 16 }]}>
      {active && <CameraView ref={camera} style={StyleSheet.absoluteFill} facing={facing} mode="video"
        videoQuality="1080p" ratio="16:9" responsiveOrientationWhenOrientationLocked={false}
        onCameraReady={() => setReady(true)} onMountError={() => { setEnabled(false); setError('Camera unavailable. Select a video from your library.'); }} />}
      ...
      {clip?.uri ? <Playback uri={clip.uri} /> : !active && <View style={s.idle}>
        {!line && <><View style={s.cameraGlyph}><Icon name="camera" size={32} /></View>
        <Text style={s.idleTitle}>Film the body</Text>
      ...
      {!clip && <View pointerEvents="none" style={s.viewfinderCaption}><Text style={s.caption}>{recording ? `${time(elapsed)} / ${time(duration)}` : active ? 'Look at the lens and speak clearly.' : Platform.OS === 'web' ? 'Camera recording is available in the mobile app' : 'Camera & microphone stay off until you enable them'}</Text></View>}
    </View>
    </View>
    {clip ? <View style={s.reviewControls}>
      {!!movement && <View><Text style={s.muted}>Review the physical action. Enter its start time in seconds.</Text>
        <TextInput accessibilityLabel="Physical action start seconds" keyboardType="decimal-pad" value={actionStart} onChangeText={setActionStart}
          placeholder="0.0" placeholderTextColor="#aaa" style={{ color: '#fff', padding: 10, borderWidth: 1, borderColor: '#777' }} /></View>}
      ...
      <View style={s.reviewRow}><Pressable accessibilityRole="button" disabled={saving} onPress={() => { setClip(undefined); setReady(false); setError(''); }} style={s.retake}><Text style={s.retakeText}>Retake</Text></Pressable><Pressable accessibilityRole="button" disabled={saving} onPress={() => void confirm()} style={s.confirm}><Text style={s.confirmText}>{saving ? 'Saving recording…' : confirmLabel} →</Text></Pressable></View>
...
export function BodyCaptureScreen({ onConfirm }: { onConfirm: (clip: Clip) => Promise<void> }) {
...
export function HookCaptureScreen({ hooks, includePhysical, takes, onSaveTake, onBack, onComplete }: {
  ...
  onSaveTake: (take: HookTake) => Promise<void>; onBack: () => void; onComplete: (lastTake: HookTake) => void;
}) {
  ...
  return <Capture key={`${hook.id}:${fingerprint}`} title="Film your hooks" subtitle={`Hook ${index + 1} of ${hooks.length} · ${completed} saved`} line={hook.line} movement={includePhysical ? hook.movement : undefined} onBack={onBack} initialClip={matchingTake(hook)?.clip}
    ...
    onUse={async clip => { const lastTake = { hookId: hook.id, fingerprint, clip }; await onSaveTake(lastTake); if (index < hooks.length - 1) setIndex(index + 1); else onComplete(lastTake); }}
...
const s = StyleSheet.create({
  ...
  frameSpace: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  viewfinder: { backgroundColor: '#202020', overflow: 'hidden', borderRadius: 16 },
